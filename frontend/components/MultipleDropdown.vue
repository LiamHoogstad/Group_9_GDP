...
export default {
  ...
  watch: {
    alreadySelectedOptions() {
      this.selectedOptions = this.alreadySelectedOptions
  ...
  computed: {
    buttonText() {
      return this.selectedOptions.length > 0
        ? `${this.valueName}: ${this.selectedOptions.join(", ")}`
        : `${this.valueName}`;
    },
  },
  methods: {
    toggleDropdown(event) {
      if (
        (event.type == "blur" && !this.$el.contains(event.relatedTarget)) ||
        event.type == "click"
      ) {
        this.$el.getElementsByTagName("ul")[0].style.zIndex = this
          .isDropdownOpen
          ? "1"
          : "10";
  ...
  <div class="multiple-dropdown">
      ...
      :style="{
        color:
          selectedOptions.length > 0
            ? 'var(--dd-active-text-colour)'
            : 'var(--dd-text-colour)',
        backgroundColor:
          selectedOptions.length > 0
            ? 'var(--dd-active-background-colour)'
            : 'var(--dd-background-colour)',
      }"
    >
    ...
    <ul
      :style="{
        ...
        boxShadow: this.isDropdownOpen
          ? '0em 0.05em 0.5em 0.01em var(--colour-dropshadow)'
          : 'none',
      }"
    >
      <button
      ...
      >
        <i
          ...
          :style="{
            ...
            marginTop: '0.3em',
            color:
              this.isDropdownOpen && selectedOptions.includes(option.value)
                ? 'var(--dd-text-colour)'
                : '#00000000',
          }"
        />
